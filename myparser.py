...
class Parser:
    ...
    def statment(self):
        ...
        # if conditions
            ...
            while not self.checkToken(Tokens.END):
                if self.checkToken(Tokens.EOF):
                    ...
                    return
                if self.checkToken(Tokens.ELSE):
                    self.nextToken()
                    self.nlPlus()
                    self.emiter.emitLine("else {")
                    while not self.checkToken(Tokens.END):
                        if self.checkToken(Tokens.EOF):
                            self.abort()
                            return
        ...
        #loops repeat - until #not sure
            ...
            print("matched with until")
            self.emiter.emit("while(!(")
            self.comaprison()
            self.match(Tokens.SEMICOLON)
            print("finished comaprison")
            self.emiter.emitLine("));")
            self.nlPlus()
            print("finished new line plus")
        ...
        #assining variables
            ...
            print("self.abort {}".format(self.error))
            print("why")
            return
        self.nl()
    ...
    def primary(self):
        if self.checkToken(Tokens.NUMBER):
            ...
            self.emiter.emit(self.curToken.tokenText)
            self.nextToken()
        elif self.checkToken(Tokens.LEPRAC):
            self.emiter.emit(self.curToken.tokenText)
            self.nextToken()
            self.expersion()
            self.emiter.emit(self.curToken.tokenText)
            self.match(Tokens.RIPRAC)
        else:
            self.abort("Unexpected token at " + self.curToken.tokenText)
            return
    
    def nlPlus(self):
        print("in new line plus")
